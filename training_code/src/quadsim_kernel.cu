...
__device__ scalar_t raycast_scene(
    ...
    int batch_base,
    scalar_t max_range,
    scalar_t other_drone_radius,
    int n_drones_per_group,
    bool include_ground) {
    scalar_t min_dist = max_range;
    scalar_t  t = (-1 - oz) / dz;
    if (include_ground && t > 0) min_dist = min(min_dist, t);
...
__global__ void render_cuda_kernel(
    ...
    scalar_t min_dist = raycast_scene(
        balls,
        cylinders,
        cylinders_h,
        voxels,
        ...
        B,
        batch_base,
        static_cast<scalar_t>(100),
        static_cast<scalar_t>(0.15),
        n_drones_per_group,
        true);
...
template <typename scalar_t>
__global__ void render_lidar_cuda_kernel(
    torch::PackedTensorAccessor<scalar_t,3,torch::RestrictPtrTraits,size_t> scan,
    torch::PackedTensorAccessor<scalar_t,3,torch::RestrictPtrTraits,size_t> balls,
    torch::PackedTensorAccessor<scalar_t,3,torch::RestrictPtrTraits,size_t> cylinders,
    torch::PackedTensorAccessor<scalar_t,3,torch::RestrictPtrTraits,size_t> cylinders_h,
    torch::PackedTensorAccessor<scalar_t,3,torch::RestrictPtrTraits,size_t> voxels,
    ...
    const int c = blockIdx.x * blockDim.x + threadIdx.x;
    const int B = scan.size(0);
    const int H = scan.size(1);
    const int V = scan.size(2);
    if (c >= B * H * V) return;
    ...
    const scalar_t az = static_cast<scalar_t>(2.0) * pi * (static_cast<scalar_t>(h) / static_cast<scalar_t>(H));
    scalar_t elev_deg;
    if (V <= 1) {
        elev_deg = static_cast<scalar_t>(0);
    } else {
        elev_deg = static_cast<scalar_t>(vfov_min_deg) +
            (static_cast<scalar_t>(vfov_max_deg - vfov_min_deg) * static_cast<scalar_t>(v)) /
            static_cast<scalar_t>(V - 1);
    }
    const scalar_t elev = elev_deg * pi / static_cast<scalar_t>(180.0);
    const scalar_t ce = cos(elev);
    const scalar_t se = sin(elev);
    const scalar_t ca = cos(az);
    const scalar_t sa = sin(az);

    const scalar_t dx =
        ce * ca * R[b][0][0] + ce * sa * R[b][0][1] + se * R[b][0][2];
    const scalar_t dy =
        ce * ca * R[b][1][0] + ce * sa * R[b][1][1] + se * R[b][1][2];
    const scalar_t dz =
        ce * ca * R[b][2][0] + ce * sa * R[b][2][1] + se * R[b][2][2];
    ...
    scan[b][h][v] = raycast_scene(
        ...
        B,
        batch_base,
        static_cast<scalar_t>(lidar_range),
        static_cast<scalar_t>(drone_radius),
        n_drones_per_group,
        true);
}
...
void render_lidar_cuda(
    ...
    float vfov_max_deg) {
    const int threads = 1024;
    size_t state_size = scan.numel();
    const dim3 blocks((state_size + threads - 1) / threads);

    AT_DISPATCH_FLOATING_TYPES(scan.scalar_type(), "render_lidar_cuda", ([&] {
        render_lidar_cuda_kernel<scalar_t><<<blocks, threads>>>(
            scan.packed_accessor<scalar_t,3,torch::RestrictPtrTraits,size_t>(),
            balls.packed_accessor<scalar_t,3,torch::RestrictPtrTraits,size_t>(),
            cylinders.packed_accessor<scalar_t,3,torch::RestrictPtrTraits,size_t>(),
            cylinders_h.packed_accessor<scalar_t,3,torch::RestrictPtrTraits,size_t>(),
            voxels.packed_accessor<scalar_t,3,torch::RestrictPtrTraits,size_t>(),
