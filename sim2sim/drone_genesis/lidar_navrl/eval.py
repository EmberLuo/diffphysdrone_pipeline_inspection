...
from sim2sim.drone_genesis.lidar_navrl.env import NavEnv
from training_code.training_tasks.lidar_navrl.model import LidarNavRLModel as Model
...
def parse_args():
    parser = argparse.ArgumentParser(description="Genesis lidar_navrl nav eval")
...
def run_episode(
    ...
    lidar_fps = int(cfg.get("policy_eval", {}).get("control_hz", 15))
    lidar_recorder = VideoRecorder(log_dir / "lidar.mp4", width=lidar_h, height=lidar_w * env.num_agents, fps=lidar_fps)
    ...
    for step_idx in range(max_steps):
        ...
        if control_elapsed >= control_dt - 1e-9:
            ...
            x = scan  # lidar scan is already in the correct format for the model
            ...
            if args.trace_policy and (policy_updates % max(1, args.trace_stride) == 0):
                ...
                for i, name in enumerate(names):
                    rec = {
                        ...
                        "wall_time": wall_time,
                        "drone": name,
                        "lidar_mean": float(x_cpu[i].mean().item()),
                        "lidar_max": float(x_cpu[i].max().item()),
                        "lidar_min": float(x_cpu[i].min().item()),
...
def main():
    ...
    if device.type != "cuda":
        raise RuntimeError("CUDA is required for lidar_navrl Genesis evaluation.")
