...
from sim2sim.drone_genesis.depth_camera.swarm.env import SwarmEnv
...
def _ensure_cv2_qt_fonts(cv2_mod) -> None:
    try:
        ...
        # Non-fatal: depth preview can still work even if font symlink fails.
        return


def parse_args():
    parser = argparse.ArgumentParser(description="Genesis swarm swap eval aligned with AirSim swarm/eval.py")
    parser.add_argument("--config", type=str, default=str(THIS_DIR / "config" / "swarm_eval.yaml"))
    parser.add_argument("--resume", type=str, default="checkpoint0004.pth")
    parser.add_argument("--env", type=str, default="swap")
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Arrival distance threshold in meters (AirSim swap eval uses 1.0m).",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Comma-separated subset of agents to run, e.g. 'drone_1,drone_2,drone_3' or '1,2,3'.",
    )
...
def _apply_agent_subset(cfg: dict, env_name: str, subset_spec: str) -> list[str] | None:
    spec = (subset_spec or "").strip()
    if not spec:
        return None

    layout_agents = cfg["task"]["layouts"][env_name]["agents"]
    all_names = [a["name"] for a in layout_agents]
    selected_names = set()
    ...
    for tok in tokens:
        if tok.isdigit():
            idx = int(tok)
            if idx < 1 or idx > len(layout_agents):
                raise ValueError(f"agent index out of range: {idx} (valid 1..{len(layout_agents)})")
            selected_names.add(layout_agents[idx - 1]["name"])
            continue

        if tok in all_names:
            selected_names.add(tok)
            continue

        if tok.startswith("drone_") and tok[6:].isdigit():
            if tok in all_names:
                selected_names.add(tok)
                continue

        raise ValueError(
            f"unknown agent token: '{tok}'. "
            f"Use names in {all_names} or 1-based indices like '1,2,3'."
        )

    filtered = [a for a in layout_agents if a["name"] in selected_names]
    if not filtered:
        raise ValueError("agent_subset produced an empty layout.")

    cfg["task"]["layouts"][env_name]["agents"] = filtered
...
def run_episode(
    ...
    if warmup_steps > 0:
        # Align hidden-state initialization with AirSim eval to avoid first-step GRU transient.
    ...
    # Use time accumulation instead of integer step downsampling so mean control rate
    # stays at control_hz even when control_dt is not an integer multiple of env.dt.
    ...
    for step_idx in range(max_steps):
        ...
        if control_elapsed >= control_dt - 1e-9:
            ...
            # AirSim-equivalent conversion: acceleration setpoint -> roll/pitch/yaw/throttle.
        ...
        # Run attitude/rate inner-loop at physics step rate using last policy setpoint.
        # Mimic AirSim's "moveToPositionAsync(..., 0.5)" after arrival: keep tracking final goal at low speed.
...
def main():
    ...
    selected = _apply_agent_subset(cfg, args.env, args.agent_subset)
    if selected is not None:
        print(f"agent_subset active: {selected}")

    gs.init(logging_level="error")

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    if device.type != "cuda":
        raise RuntimeError("CUDA is required for swarm Genesis evaluation.")
    ...
    env = SwarmEnv(cfg=cfg, show_viewer=args.show_viewer, device=str(device))
