...
class LidarSensor(BaseSensor):
    ...
    def __init__(
        self, env, env_cfg, sensor_config: LidarConfig, num_sensors=1, device="cuda:0"
    ):
        self.env = env
    ...
    def _setup_height_scanner_parameters(self):
        ...
        if self.sensor_cfg.height_scanner_resolution <= 0:
            raise ValueError(
                f"Height scanner resolution must be greater than 0. Received: {self.sensor_cfg.height_scanner_resolution}"
            )

        if self.sensor_cfg.height_scanner_ordering not in ["xy", "yx"]:
            raise ValueError(
                f"Height scanner ordering must be 'xy' or 'yx'. Received: '{self.sensor_cfg.height_scanner_ordering}'"
            )
    ...
    def initialize_ray_vectors(self):
        ...
        print(
            f"Ray vectors initialized for {self.sensor_cfg.sensor_type.value} "
    ...
    def _initialize_grid_rays(self):
        ...
        for i in range(self.num_vertical_lines):
            for j in range(self.num_horizontal_lines):
                ...
                ray_vectors[i, j, 0] = math.cos(azimuth_angle) * math.cos(
                    elevation_angle
                )
                ray_vectors[i, j, 1] = math.sin(azimuth_angle) * math.cos(
                    elevation_angle
                )
    ...
    def _initialize_height_scanner_rays(self):
        ...
        self.num_horizontal_lines = (
            int(size_x / resolution) + 1
        )  # +1 to include endpoints
        self.num_vertical_lines = int(size_y / resolution) + 1

        print(
            f"Height scanner setup: {self.num_horizontal_lines}x{self.num_vertical_lines} grid, {resolution}m spacing"
        )
        ...
        base_direction = torch.tensor(
            direction, device=self.device, dtype=torch.float32
        )
        ...
        ray_origins = torch.zeros(
            (self.num_horizontal_lines, self.num_vertical_lines, 3),
            device=self.device,
            dtype=torch.float32,
        )
        ...
        height_above_ground = (
            self.sensor_cfg.height_scanner_height_above_ground
        )  # Configurable height above ground
        offset_x, offset_y = (
            self.sensor_cfg.height_scanner_offset
        ...
        ray_directions[..., :] = torch.tensor(
            list(self.sensor_cfg.height_scanner_direction), device=self.device
        )
        ...
        print(
            f"Height scanner rays initialized: {self.num_horizontal_lines}x{self.num_vertical_lines} = {self.num_horizontal_lines * self.num_vertical_lines} rays"
        )
        print(
            f"Grid spans: X=[{grid_x.min():.2f}, {grid_x.max():.2f}], Y=[{grid_y.min():.2f}, {grid_y.max():.2f}]"
        )
    ...
    def _initialize_pattern_rays(self):
        ...
        rays_theta_tensor = torch.tensor(
            rays_theta, dtype=torch.float32, device=self.device
        )
        rays_phi_tensor = torch.tensor(
            rays_phi, dtype=torch.float32, device=self.device
        )
    ...
    def _generate_ray_angles(self):
        ...
        if self.sensor_cfg.is_livox_sensor and isinstance(
            self.ray_generator, LivoxGenerator
        ):
            return self.ray_generator.sample_ray_angles()
        elif self.sensor_cfg.is_spinning_lidar and isinstance(
            self.ray_generator, SpinningLidarGenerator
        ):
        ...
        raise ValueError(
            f"Cannot generate ray angles for sensor type: {sensor_type_str}"
        )
    ...
    def create_render_graph_pointcloud(self):
        ...
        if "cuda" not in str(self.device).lower():
            print(
                f"Note: Graph capture not available on {self.device}, using direct kernel launch"
            )
        ...
        try:
            ...
            kernel_type = (
                "height_scanner" if self.sensor_cfg.is_height_scanner else "regular"
            )
            print(
                f"Render graph created for {self.sensor_cfg.sensor_type.value} lidar ({kernel_type} kernel)"
            )
    ...
    def init_tensors(self):
        ...
        self.lidar_pixels_tensor = torch.zeros_like(
            self.lidar_tensor, device=self.device
        )
        ...
        if self.sensor_cfg.is_height_scanner:
            ...
            ray_origins_expanded = (
                self.ray_origins.unsqueeze(0)
                .unsqueeze(0)
                .expand(self.num_envs, self.num_sensors, -1, -1, -1)
            )
            ray_directions_expanded = (
                self.ray_directions.unsqueeze(0)
                .unsqueeze(0)
                .expand(self.num_envs, self.num_sensors, -1, -1, -1)
            )

            self.height_scanner_ray_origins = wp.from_torch(
                ray_origins_expanded.view(
                    self.num_envs,
                    self.num_sensors,
                    self.num_vertical_lines,
                    self.num_horizontal_lines,
