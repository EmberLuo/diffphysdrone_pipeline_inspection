...
class GenesisG1LidarVisualizer:
    ...
    def __init__(self, 
        ...
        self.sensor_translation = torch.tensor([0.0, 0.0, 0.0], device=self.device)
    ...
    def _find_g1_urdf(self) -> Optional[str]:
        """Find G1 URDF file"""
        paths = [
            "assets/drone_ex1/drone_ex1.urdf",
    ...
    def _setup_lidar_sensor(self):
        ...
        try:
            ...
            sensor_config = LidarConfig(
                ...
                pointcloud_in_world_frame=True,  # CRITICAL: Return world coordinates directly
    ...
    def step(self) -> Tuple[Optional[torch.Tensor], Optional[torch.Tensor]]:
        ...
        if self.lidar_sensor is not None:
            try:
                ...
                # Get lidar data - now returns world coordinates directly
    ...
    def _visualize_lidar_points(self):
        ...
        try:
            ...
            # CRITICAL FIX: Points are already in world coordinates, no transformation needed!
            # The LidarSensor now returns world coordinates directly due to pointcloud_in_world_frame=True
            world_points = points.view(-1, 3)
            
            # DEBUG: Print coordinate information
            if self.episode_length % 50 == 0:  # Print every 50 steps
                print(f"DEBUG - Episode {self.episode_length}:")
                print(f"  Robot position: {self.base_pos[0].tolist()}")
                print(f"  Robot orientation: {self.base_quat[0].tolist()}")
                print(f"  Point cloud shape: {points.shape}")
                if len(world_points) > 0:
                    print(f"  Point range X: [{world_points[:, 0].min().item():.2f}, {world_points[:, 0].max().item():.2f}]")
                    print(f"  Point range Y: [{world_points[:, 1].min().item():.2f}, {world_points[:, 1].max().item():.2f}]")
                    print(f"  Point range Z: [{world_points[:, 2].min().item():.2f}, {world_points[:, 2].max().item():.2f}]")
                    print(f"  Sample points: {world_points[:3].tolist()}")
            ...
            for step in range(1000):
                ...
                if step % 50 == 0 and env.robot is not None:
                    try:
                        ...
                        print("base_quat:", env.base_quat[0].tolist())
